...
class ReviewProvider extends RegAuth{
  ...
  Future<void> getTotalAppointment()async{
    final String docId = await getPreferenceId();
  ...
  Future<void> getAllReview()async{
    final String docId = await getPreferenceId();
    try{
      await FirebaseFirestore.instance.collection('AppointmentList').where('drId',isEqualTo: docId).orderBy('reviewTimeStamp',descending: true).get().then((snapshot){
