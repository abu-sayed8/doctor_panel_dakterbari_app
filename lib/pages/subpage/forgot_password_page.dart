...
class _ForgotPasswordState extends State<ForgotPassword> {
  ...
  void _initializeDoctorData(DoctorProvider provider) async {
    provider.doctorDetails.phone = '';
  }

  @override
  Widget build(BuildContext context) {
    final DoctorProvider provider = Provider.of<DoctorProvider>(context);
    if (provider.doctorDetails.phone == null) _initializeDoctorData(provider);
    return Scaffold(
      key: _scaffoldKey,
  ...
  Widget _bodyUI(DoctorProvider provider) {
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: TextFormField(
                        ...
                        onChanged: (val) =>
                            setState(() => provider.doctorDetails.phone = val),
  ...
  Widget _countryCodePicker(DoctorProvider provider) {
    return CountryCodePicker(
      comparator: (a, b) => b.name.compareTo(a.name),
      onChanged: (val) {
        provider.doctorDetails.countryCode = val.dialCode;
        //print(countryCode);
      },
      onInit: (code) {
        provider.doctorDetails.countryCode = code.dialCode;
        //print(countryCode);
      },
      favorite: ['+880', 'BD'],
      initialSelection: 'BD',
  ...
  void _checkValidity(DoctorProvider provider) async {
    if (provider.doctorDetails.phone.isNotEmpty) {
      provider.loadingMgs = "Verifying...";
      showLoadingDialog(context, provider);
      final bool isRegistered = await provider.isDoctorRegistered(
          provider.doctorDetails.countryCode + provider.doctorDetails.phone);
      if (isRegistered) {
        _OTPVerification(provider);
      } else {
        Navigator.pop(context);
        showSnackBar(_scaffoldKey, "No doctor found of this phone number",
            Colors.deepOrange);
      }
    } else
  ...
  void _OTPDialog(DoctorProvider provider) {
    showDialog(
        context: context,
  ...
  // ignore: non_constant_identifier_names
  Future<void> _OTPVerification(DoctorProvider provider) async {
